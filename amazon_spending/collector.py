...
def _parse_related_transactions(
    ...
    source_url: str,
) -> list[ParsedAmazonTransaction]:
    txns: list[ParsedAmazonTransaction] = []
...
def _reconcile_orders_and_shipments(conn: sqlite3.Connection, orders: list[ParsedOrder]) -> dict[str, int]:
    ...
    for o in orders:
        ...
        if existing_order is None:
            conn.execute(
                """
                INSERT INTO orders (order_id, order_date, order_url, order_total_cents, tax_cents, shipping_cents, payment_last4)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    o.order_id,
...
def _reconcile_items(conn: sqlite3.Connection, items: list[ParsedItem]) -> dict[str, int]:
    ...
    for i in items:
        ...
        existing_item = conn.execute(
            """
            SELECT order_id, shipment_id, amazon_transaction_id, title, quantity, item_subtotal_cents, item_tax_cents
            FROM order_items
            WHERE item_id = ?
            """,
            (i.item_id,),
        ...
        if existing_item is None:
            conn.execute(
                ...
                INSERT INTO order_items (
                    item_id, order_id, shipment_id, amazon_transaction_id, title, quantity, item_subtotal_cents, item_tax_cents
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
            ...
            changed = (
                ...
                or existing_item["amazon_transaction_id"] is not None
...
def _reconcile_amazon_transactions(
    ...
    parsed_by_order: dict[str, list[ParsedAmazonTransaction]],
) -> dict[str, int]:
    stats = {
    ...
    for order_id, parsed_txns in parsed_by_order.items():
        desired_ids = {t.amazon_txn_id for t in parsed_txns}
        existing_ids = {
            row["amazon_txn_id"]
            for row in conn.execute(
                "SELECT amazon_txn_id FROM amazon_transactions WHERE order_id = ?",
                (order_id,),
            ).fetchall()
        }
        stale_ids = sorted(existing_ids - desired_ids)
        if stale_ids:
            ...
            conn.execute(
                f"UPDATE order_items SET amazon_transaction_id = NULL WHERE amazon_transaction_id IN ({placeholders})",
                (*stale_ids,),
            )
            conn.execute(
                f"DELETE FROM order_item_transactions WHERE amazon_txn_id IN ({placeholders})",
                (*stale_ids,),
            )
            conn.execute(
                f"DELETE FROM amazon_transactions WHERE order_id = ? AND amazon_txn_id IN ({placeholders})",
                (order_id, *stale_ids),
            )
            ...
            if existing is None:
                conn.execute(
                    ...
                    INSERT INTO amazon_transactions (
                        amazon_txn_id, order_id, transaction_tag, txn_date, amount_cents, payment_last4, raw_label, source_url
                    )
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
...
def _reconcile_item_transaction_links(
    ...
    txns_by_order: dict[str, list[ParsedAmazonTransaction]],
) -> int:
    links_written = 0
    for order_id, items in items_by_order.items():
        ...
        conn.execute("UPDATE order_items SET amazon_transaction_id = NULL WHERE order_id = ?", (order_id,))
        ...
        if len(txns) == 1:
            ...
            for item in items:
                conn.execute(
                    """
                    INSERT OR REPLACE INTO order_item_transactions (item_id, amazon_txn_id, allocated_amount_cents, method)
                    VALUES (?, ?, ?, ?)
                    """,
                    (item.item_id, txn.amazon_txn_id, item.item_subtotal_cents, "single_transaction"),
                )
                conn.execute(
                    "UPDATE order_items SET amazon_transaction_id = ?, updated_at = datetime('now') WHERE item_id = ?",
                    (txn.amazon_txn_id, item.item_id),
                )
                links_written += 1
            continue
        ...
        for item in items:
            ...
            for idx, txn in enumerate(txns):
                ...
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?)
                    """,
                    (item.item_id, txn.amazon_txn_id, allocated, "proportional_multi_transaction"),
                )
                links_written += 1
            ...
            try:
                ...
                if parsed_order:
                    ...
                    if _in_date_range(parsed_order.order_date, start_date, end_date):
                        ...
                        parsed_txns: list[ParsedAmazonTransaction] = []
                        try:
